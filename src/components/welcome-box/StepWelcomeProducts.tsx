...
import { ProductItem, DesignOption } from './WelcomeBoxTypes';

interface StepWelcomeProductsProps {
...
export default function StepWelcomeProducts({
  catalogProducts,
  selectedProducts,
  onAddProduct,
  onRemoveProduct,
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <p className="text-center text-espresso-light mb-2 max-w-lg mx-auto">
        Select curated items to include in every welcome box.
      </p>
      <p className="text-center text-xs text-gold uppercase tracking-widest mb-8 font-semibold">
        ✦ Each item you select will be included in all {quantity} boxes
      </p>
        ...
        <div className="mb-8 bg-white/60 border border-gold/20 rounded-2xl px-6 py-4">
          <div className="flex items-center gap-4">
            <img
              ...
              className="w-16 h-16 rounded-xl object-cover border border-gold-pale/30"
              onError={(e) => {
                (e.target as HTMLImageElement).style.display = 'none';
              }}
            />
            <div>
              <p className="font-sans text-xs uppercase tracking-wider text-gold font-semibold mb-1">
                Your Lid Design
              </p>
              <p className="font-serif text-lg text-espresso">{selectedDesign.name}</p>
              {namesOrInitials && (
                <p className="text-sm font-sans text-espresso/60">
                  {namesOrInitials} · {eventDate}
                </p>
              )}
            </div>
          </div>
          <p className="text-[11px] font-sans text-espresso/40 mt-3">
            All {quantity} boxes will feature this design on the lid. Every item below will be included in each box.
          </p>
        </div>
      )}
            ...
            {hasDiscount ? (
              <div className="flex flex-col items-center">
                ...
                <span className="font-serif text-xl text-gold font-semibold">
                  ${(selectedProducts.reduce((sum, p) => sum + p.price, 0) * (1 - discountPercent / 100)).toFixed(2)}
                </span>
              </div>
            ) : (
